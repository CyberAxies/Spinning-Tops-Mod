...
namespace SpinningTopsMod.Content.Projectiles
{
    ...
    public class MetalTop : ModProjectile
    {
        ...
        {
            Projectile.width = 20; // The width of the projectile hitbox
            ...
            Projectile.timeLeft = 600 * 2; // How long the projectile lasts before disappearing
            ...
            Projectile.localNPCHitCooldown = 22; // Set the cooldown for hitting NPC
            Projectile.tileCollide = true; // Enable tile collision
            Projectile.light = 0.3f; // Add a light effect to the projectile
        }
        int maxHits = 12; // Set the maximum number of NPCs to hit before destroying the projectile
        ...
        public override void AI()
        {
            ...
            {
               Dust dust = Dust.NewDustPerfect(Projectile.Center, DustID.Torch, Vector2.Zero, 100, default, 1.5f);
                dust.position = Projectile.Center;
                dust.noGravity = true; // Make the dust not affected by gravity
                dust.velocity.Y = -1f; // Add a slight downward velocity to the dust
            ...
            {
                Projectile.velocity.X *= 0.99f; // Reduce X velocity by 1% , 8-ish times per second
            }
            if (Projectile.ai[1] == 1)
            {
                Projectile.velocity.Y *= 4.3f; // Increase the Y velocity on the first frame
            }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            // apply debuff to the NPC
            target.AddBuff(BuffID.OnFire, 60 * 5); // Apply the On Fire! buff for 60 frames (1 second)
        }
