...
namespace SpinningTopsMod.Content.Projectiles
{
    ...
    public class SnowTop : ModProjectile
    {
        ...
        {
            Projectile.ai[1]++;
            Vector2 oldvelocity = Projectile.velocity; // Store the old velocity for tile collision logic
            ...
            {
                Dust fxDust = Dust.NewDustPerfect(Projectile.Center, DustID.IceTorch, Vector2.Zero, 100, default, 2f);
                fxDust.position = Projectile.Center;
                fxDust.noGravity = true; // Make the dust not affected by gravity
                fxDust.velocity.Y = -1f; // Add a slight downward velocity to the fxDust
                Vector3 DustColor = fxDust.color.ToVector3(); // Get the color of the dust
                Lighting.AddLight(Projectile.position, DustColor); // Add light to the dust
            }
            if (Projectile.ai[1] % 29 == 0)
            {
                Dust fxDust = Dust.NewDustPerfect(Projectile.Center, DustID.Snow, Vector2.Zero, 100, default, 1.2f);
                fxDust.position = Projectile.Center;
                fxDust.velocity.Y = 2f; // Add a slight downward velocity to the fxDust
            }
            ...
            /* if (Projectile.ai[1] % 15 == 0) // Apply friction every 5 frames
             {
                 Projectile.velocity.X *= 0.99f; // Reduce X velocity by 5% every 5 frames
             }*/
            if (Projectile.ai[1] == 1)
            {
                Projectile.velocity.Y *= 3f; // Increase the Y velocity the first time the AI runs
            }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                Projectile.velocity = -Projectile.velocity; // Reverse the velocity and reduce speed
            }

            target.AddBuff(BuffID.Frostburn, 300); // Apply Frostburn debuff for 5 seconds
            SoundEngine.PlaySound(SoundID.Item27, Projectile.position); // Play a sound effect
        }
