...
namespace SpinningTopsMod.Content.Projectiles
{
    ...
    public class CrimsonTop : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            int BloodRainCustomID;

            // Add a dust effect to the projectile 
            if (Projectile.ai[1] % 23 == 0) // Pretty sure 37 is prime so the blood rain desyncs and looks cooler
            {
                /*Dust dust = Dust.NewDustPerfect(Projectile.Center, DustID.Blood, Vector2.Zero, 100, default, 1.5f);
                ...
                // Replace dust with similiar projectile effect
                BloodRainCustomID = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Bottom,
                 new Vector2(0f, 10f), ProjectileID.BloodRain, (int)Math.Floor(Projectile.damage * 0.58), Projectile.knockBack / 2, Projectile.owner);

            }

            if (Projectile.ai[1] % 17 == 0)
            { 
                Dust dust = Dust.NewDustPerfect(Projectile.Center, DustID.Blood, Vector2.Zero, 100, default, 1.5f);
                Vector3 DustColor = dust.color.ToVector3(); // Get the color of the dust
                Lighting.AddLight(Projectile.position, DustColor); // Add light to the dust
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            // Bleed and bloodbutchered don't work for some reason *shrugs*
